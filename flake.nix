  ...
  outputs =
    ...
    }:
    {
      lib.mkIceDOS =
        ...
        let
          inherit (builtins) isString pathExists readFile throw;
          inherit (nixpkgs.lib) isDerivation;

          isFlake = value: (value._type or null) == "flake";

          _configRoot =
            if ((isFlake configRoot) && (pathExists "${configRoot}/flake.nix") && (pathExists "${configRoot}/config.toml"))
            then (configRoot)
            else (throw "The value of `configRoot` is invalid. Please set `configRoot = self;`.");

          _stateDir =
            if (isString stateDir)
            then (stateDir)
            else (throw "The value of `stateDir` should be a string.");

...
Alternatively, `--github-ssh` (or the `icedos.system.githubViaSsh` option) rewrites every `github:` input of the generated state flake — repositories, module inputs, channels, overlays, `extraFlakes`, `nixpkgs`, `home-manager` — to `git+ssh://git@github.com/...`, so your ssh key authenticates the fetches and the token machinery is left idle. The state flake's own inputs (e.g. `icedos` core via `icedos-core.follows`) still fetch over https with the token. The build's nix may run as root, so make sure the key is reachable there too (`ssh-add` your key, or point `GIT_SSH_COMMAND` at an agent).

Under the switch, an inline ref on a `github:` url becomes a query on the ssh url: a full 40-hex rev (any case) pins `?rev=<hash>`, a bare name pins `?ref=<name>` — which nix resolves as the `refs/heads/<name>` branch. Tags therefore must be written `refs/tags/<tag>` (e.g. `github:owner/repo/refs/tags/v1.2.3`) and short revs must be expanded to the full 40-hex hash. Rewriting `nixpkgs`/`home-manager` channels also turns their tarball fetch into a full git clone, so the first ssh build transfers noticeably more than before.

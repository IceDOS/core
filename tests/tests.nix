...
in
{
  ...
  # Regression coverage for the wrapped-tip machinery: wrap helpers and the
  # too-small fallback are wired in, and the pinned CSI sequence stays intact.
  tipsWrapGeometry =
    let
      sample = tipsSampleHead;
      countIn = needle: (builtins.length (lib.strings.splitString needle sample)) - 1;
      pin = "printf '\\033[?7l\\033[%d;1H\\033[K%b%s%b\\033[?7h'";
    in
    expectOk (
      countIn pin == 3
      && countIn "tip_rows=$(_icedos_tip_wrap_rows" == 2
      && countIn "[ \"$tip_rows\" -gt \"$((rows - 3))\" ]" == 2
      && countIn "max=$(( cols - 2 ))" == 2
      && countIn "printf '\\033[1;%dr' \"$((rows - 1))\"" == 1
      && countIn "printf '\\033[1;%dr' \"$((rows - tip_rows))\"" == 2
      && lib.strings.hasInfix "truncate=1" sample
      && !lib.strings.hasInfix "_icedos_max" sample
      && !lib.strings.hasInfix "vislen" sample
    );
}
